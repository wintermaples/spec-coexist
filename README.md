...
Claude Code 向けの **仕様駆動開発 (spec-driven development) スキルスイート** です。要件定義から基本設計・詳細設計・実装・仕様変更反映・デバッグまでの一連のワークフローを、Claude Code の [Skills](https://code.claude.com/docs/en/skills) として提供します。
...
Claude Code は `.claude/skills/` をライブリロードの対象として扱います ([公式ドキュメント](https://code.claude.com/docs/en/skills))。スキル本体をここに置くと、`SKILL.md` を編集した瞬間から次のターンに反映されるため、デバッグや試行錯誤がしやすくなります。
...
```
built /workspace/dist/spec-coexist-<version>.tar.gz
contents:
  spec-coexist/
  spec-coexist/.claude-plugin/plugin.json
  spec-coexist/skills/spec-coexist-router/SKILL.md
  spec-coexist/skills/creating-requirements/SKILL.md
  …
```

### バージョンを上げる

配布バージョンの単一の真実源 (single source of truth) は `packaging/spec-coexist/.claude-plugin/plugin.json` の `version` フィールドです。リリース時はここを書き換えてからパッケージングスクリプトを実行してください。
...
生成された `dist/spec-coexist-<version>.tar.gz` を展開し、Claude Code のプラグインディレクトリに配置してください。詳しくは [Claude Code Plugins ドキュメント](https://code.claude.com/docs/en/plugins) を参照してください。

## CI ワークフロー (利用者向け)

`.claude/skills/_utils/github-workflows/` には、スキルで開発した成果物を検証するための GitHub Actions ワークフローを同梱しています。
...
- Bash (パッケージングスクリプト用)
- Python 3.8+ (Visual Companion 機能を使う場合のみ。標準ライブラリのみで動作するため追加インストールは不要)
