...
Several skill names in this suite (`systematic-debugging`, `requesting-code-review`, `receiving-code-review`, `verification-before-completion`, `writing-plans`-adjacent work, etc.) **collide** with skill names in `superpowers`. In repositories where both suites are installed, an unqualified reference can route to either implementation, breaking the suite's **independence** guarantee (see `independence.md`).
...
- The Skill Inventory table in `using-spec-coexist/SKILL.md`.
...
When authoring a new skill via `spec-coexist:authoring-spec-coexist-skill`, step 7 (register in inventory) **MUST** use the prefixed form.
