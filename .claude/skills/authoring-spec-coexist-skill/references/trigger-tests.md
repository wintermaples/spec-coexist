...
```json
{
  "id": "authoring-spec-coexist-skill-pos-1",
  "skill": "authoring-spec-coexist-skill",
...
Most false positives come from over-general trigger phrases matching adjacent territory. A negative case like `"要件を少し直したい"` for `creating-requirements` (it should route to `revising-spec`, not `creating-requirements`) is worth more than five obvious positive cases. Budget your attention accordingly.
