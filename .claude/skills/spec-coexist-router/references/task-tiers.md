...
**必要スキル**: `spec-coexist:fast-path`（T0 手順 — 直接編集のみ。追加プロセスなし）。
...
**最低限**: 既存 spec との整合確認、TDD、verification、pre-review-self-check → code-review。

### T3: large

**例**: 新サブシステム、サブシステム横断機能、アーキテクチャ変更、振る舞いの根本変更。

**必要スキル**: フルパイプライン (exploring-problem-space → creating-requirements → creating-basic-design → [creating-detail-design 推奨] → implementing-from-spec → pre-review-self-check → code-review-loop → verification)。
**スキップ可能**: なし。
