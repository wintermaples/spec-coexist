...
3. **Write changelog entry.** Append to `docs/changelog/{subsystem-or-main}-{YYYY-MM-DD}.md` a short entry: subject, rationale, acceptance bullets covered, links to RED evidence and verification evidence. For nested subsystems, use the `~`-separated qualified ID as the subsystem identifier (e.g. `docs/changelog/001_common~001_notification-2026-04-12.md`). **MUST** be committed in the same logical unit as the code change, not a trailing "docs" commit.
