...
- One acceptance bullet from the basic design **MUST** drive exactly one RED test.
- The test **MUST** fail for the right reason — an assertion failure, not an import or syntax error.
- The failure **MUST** be captured by `scripts/record_red_phase.sh <slug> -- <cmd>` as a `tdd-red` evidence record.
...
- The pass **MUST** be captured by `scripts/record_green_phase.sh <slug> -- <cmd>` using the **same slug** as RED. Pair correlation is by slug.
