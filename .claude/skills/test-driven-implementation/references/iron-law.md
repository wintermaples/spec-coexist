...
The `tdd-red` evidence commit **MUST** be a git **ancestor** of the `tdd-green` commit. This is enforced by `_shared/scripts/verify_evidence.sh` via `git merge-base --is-ancestor`.

Concretely:

1. Write the failing test and commit it — this produces `tdd-red` evidence with `commit_sha`.
2. Write production code to make the test pass, then commit — this produces `tdd-green` evidence with `commit_sha`.
