...
- The skill is an **orchestration trigger** whose whole job is to route to other skills (e.g. `spec-coexist-router`).
...
|---|---|---|
| `spec-coexist-router` | false | Pure router; nothing to do if invoked directly. |
...
| `code-review-loop` | true | Same logic as verification. |
| `pre-review-self-check` | true | User-meaningful ("self review before submitting"). |
