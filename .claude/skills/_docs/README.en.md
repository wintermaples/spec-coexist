...
A GitHub Actions workflow is included to verify artifacts produced by the skills — document link integrity, evidence completeness, REQ-ID traceability, and more.

### Setup

Copy the workflow file from `.claude/skills/_utils/github-workflows/` into your project's `.github/workflows/`:
...
| File | What it checks |
| --- | --- |
| `spec-coexist.yml` | Tier detection, evidence schema validation, evidence completeness, REQ-ID traceability, document link integrity |

> **Note**: This workflow validates artifacts generated by the skills (`docs/`, `.spec-coexist/evidence/`, etc.) — it is NOT a CI for the skills themselves.
