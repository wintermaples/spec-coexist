...
このリポジトリを clone するだけで `.claude/skills/` 配下のスキルが自動認識されます。
...
すべてのユーザーメッセージは **spec-coexist-router** によって4段階に分類されます。ティアに応じて必要なプロセスが異なります。
...
要件定義書を新規作成します。全体要件 (`docs/main-requirements.md`) またはサブシステム単位で作成可能です。
...
基本設計書を新規作成します。対応する要件定義書が存在しない場合は作成を中断します。
...
要件定義 + 基本設計に基づいてコードを実装します。内部で test-driven-implementation を呼び出し、TDD で進めます。
...
T0/T1 の軽量タスク専用。仕様文書を要求せず、最短経路で完了します。
...
要件定義・基本設計の改訂、および仕様変更の実装反映を統合的に扱います。
...
仮説駆動のデバッグプロセス。修正案を出す前に、証拠収集と仮説検証を行います。
...
コードレビューの依頼・フィードバック受領・修正を一つのループで処理します。
...
大規模プロジェクトでは、機能をサブシステムに分割して開発できます。
...
要件定義や基本設計の対話を視覚的に進めるための軽量 HTTP サーバです。Mermaid 図のリアルタイムプレビューなどを提供します。

**起動方法:**

スキルの実行中に自動的に起動されます。手動で起動する場合:
...
多くの場合、自然な日本語や英語で話しかけるだけで適切なスキルが選択されます。スキル名を覚える必要はありません。
...
spec-coexist の核心は「仕様と実装が常に同期している」ことです。実装だけを変更したくなった場合でも、まず仕様を更新してから実装に反映するフローを推奨します。

### ガードレールを信頼する

各スキルには安全装置が組み込まれています。
- 既存ドキュメントの上書き防止
- 要件定義なしの基本設計作成防止
- テストなしのプロダクションコード作成防止
- 検証なしの完了宣言防止

これらの HALT は意図的な設計です。回避せず、指示に従ってください。
