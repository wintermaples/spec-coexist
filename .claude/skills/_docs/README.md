...
```
問題探索 → 要件定義 → 基本設計 → (任意) 詳細設計 → 実装 → レビュー → 完了
   │          │          │            │             │       │        │
   ▼          ▼          ▼            ▼             ▼       ▼        ▼
...
> 詳細設計フェーズは任意ですが、複雑な振る舞いやモジュール契約を明確化したい場合に推奨されます。
...
詳細設計書を新規作成します。Mermaid 図を中心にモジュールの振る舞いと契約を定義し、実装ブレを防ぎます。
...
**前提条件:** 対応する基本設計書が存在すること。

**配置:**
- 全体: `docs/main-detail-design/index.md`
- サブシステム: `docs/subsystems/{id}_{name}/detail-design/index.md`

**ガードレール:** 既存の詳細設計書を上書きしません。更新は `revising` スキル経由で行ってください。
...
TDD の鉄則 (失敗するテスト先行) を強制するサブスキル。プロダクションコードが書かれる前に、必ず失敗するテストが存在することを保証します。  
*`implementing-from-spec` および `revising` から自動的に呼び出されます。*
...
├── main-detail-design/            ← (任意) 全体詳細設計
│   └── index.md
└── subsystems/
    ├── 01_auth/
    │   ├── auth-requirements.md
    │   ├── auth-design.md
    │   └── detail-design/         ← (任意) サブシステム詳細設計
    │       └── index.md
    ├── 02_api/
    │   ├── api-requirements.md
    │   ├── api-design.md
